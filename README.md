# FPGA Implementation & Digital IC Design

### 🎓 Background & Inspiration
The foundation for the projects in this repository was built during an intensive internship on **Digital IC Design using Cadence Tools**. This program was hosted by the **IEEE MEC Student Branch (SB)**, conducted by the **IEEE CASS Kerala Chapter**, and held at **Govt. Model Engineering College**. The exposure to industry-standard Cadence workflows during this internship deeply influenced my approach to digital design, simulation, and hardware synthesis.

---

## 📂 Repository Contents

All source codes, testbenches, and constraint files.
The project detailes are mentioned in the `main` branch 

### Key Implementations
* **Combinational Logic Circuits:** * `Half adder`
  * `Full Adder`
  * `XOR GATE`
  * `4x1mux`
  * `Comparator`
* **Sequential Logic Circuits:** * `SISO` (Serial-In Serial-Out Shift Register)
  * `Counter`
* **Hardware Constraints:** * `boolean.xdc` (Xilinx Design Constraints file for physical pin mapping)

---

## 🛠️ Tools & Technologies

The designs in this repository were developed and simulated using the following stack:
* **Hardware Description Languages:** Verilog / SystemVerilog
* **Synthesis & Simulation:** Xilinx Vivado, Icarus Verilog (iverilog), GTKWave
* **EDA Tools:** Cadence Tools (utilized during the foundational internship)

---

## 🚀 How to Use This Repository

To run or simulate any of the codes provided in this repository:

1. **Clone the repository:**
   ```bash
   git clone [https://github.com/Annada2106/Internship_26.git](https://github.com/Annada2106/Internship_26.git)

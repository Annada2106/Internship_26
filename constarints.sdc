...
set CLOCK_PERIOD 125;
set CLOCK_NAME rtc_clk;
set SKEW_setup 1.0;
set SKEW_hold 0.5;
set MINRISE 0.2;
set MAXRISE 1.0;
...
####### CLOCK CONSTRAINTS ########
create_clock -name rtc_clk -period 125 -waveform {0 62.5} [get_ports clk_pad]


####### CLOCK SOURCE LATENCY ########
set_clock_latency -source -max 3.00 -late [get_clocks rtc_clk]
set_clock_latency -source -min 1.00 -late [get_clocks rtc_clk]
set_clock_latency -source -max 3.00 -early [get_clocks rtc_clk]
set_clock_latency -source -min 1.00 -early [get_clocks rtc_clk]


####### CLOCK TRANSITION ########
set_clock_transition -rise -min $MINRISE [get_clocks rtc_clk]
set_clock_transition -rise -max $MAXRISE [get_clocks rtc_clk]
set_clock_transition -fall -min $MINRISE [get_clocks rtc_clk]
set_clock_transition -fall -max $MAXRISE [get_clocks rtc_clk]


####### CLOCK UNCERTAINITY ########
set_clock_uncertainty -setup $SKEW_setup [get_clocks rtc_clk]
set_clock_uncertainty -hold $SKEW_hold [get_clocks rtc_clk]
...
####### VIRTUAL CLOCK ########
create_clock -name vir_main_clk -period 125
...
set_input_delay -clock vir_main_clk -max 20 [get_ports -filter "direction == in && name != clk_pad"]


####### OUTPUT DELAYS  RELATIVE TO VIRTUAL CLOCK ########
set_output_delay -clock vir_main_clk -min 2 [get_ports -filter "direction == out"] 
set_output_delay -clock vir_main_clk -max 20 [get_ports -filter "direction == out"]
